...
int Forest::getClassNumberFromHistogram(int numberOfClasses, std::vector<float> histog) {
    int dist = -1;

    std::vector<float>::iterator maxElement;
    maxElement = std::max_element(histog.begin(), histog.end());

    dist = std::distance(histog.begin(), maxElement);
...
const float* Forest::EstimateClass(const Feature &feature_vec){
  ...
  for(unsigned int t=0; t<trees.size(); ++t){
    ...
    // for(int c=0; c<numClass; ++c){
    //   std::cout << histogram[c] << ", ";
    // }

    std::vector<float> my_vector {histogram, histogram + numClass};
    // for (std::vector<float>::iterator it=my_vector.begin(); it!=my_vector.end(); ++it)
    //   std::cout << ' ' << *it;

    int classification = getClassNumberFromHistogram(numClass, my_vector);
  ...
  treeClassifications = classifications;
